...
?>  按照条目对迁移账户进行所需权限开放
 ...
 |   | 目标云端 | 权限          | 权限条目        | 备注          |
 | - | -------- | --------------- | ------------------- | --------------- |
 | 1 | 腾讯云 | 云服务器（CVM） | QcloudCVMFullAccess | 云服务器（CVM）全读写访问权限，包括CVM及相关CLB、VPC、监控权限 |
 | 2 |          | 云服务器（CVM）财务 |QcloudCVMFinanceAccess | 云服务器（CVM）财务权限 |

!> 1.如该账号为主账号，默认已赋予以上权限，请跳过此步骤；</br>
2.如以上权限未开放，请参考腾讯云文档中[子用户权限设置](https://cloud.tencent.com/document/product/598/36256) 中有关为RAM用户、用户组分配授权策略的说明；
